...
string ROBOT_EE_FORCE_KEY = "sai2::HapticApplications::02::robot::ee_force";
...
int prev_delayed_force_space_dimension = 0;
...
int main(int argc, char* argv[]) {
	...
	Vector3d sensed_force_robot_frame = Vector3d::Zero();
	Vector3d sensed_force_offset = Vector3d::Zero();
	...
	// read internal robot controller state from redis
    redis_client_local.addEigenToReadCallback(0, ROBOT_EE_FORCE_KEY, sensed_force_robot_frame);
	...
	while (runloop) {
		...
		if(state == INIT) {
			...
			if(prev_delayed_force_space_dimension == 0 && delayed_force_space_dimension > 0) {
				// sensed_force_offset = sensed_force_robot_frame;
			}
			else {
				sensed_force_offset.setZero();
			}

			// Vector3d desired_force = k_vir * delayed_sigma_force * (proxy_position_device_frame - device_position);
			Vector3d desired_force = k_vir * delayed_sigma_force * teleop_task->_Rotation_Matrix_DeviceToRobot * -(sensed_force_robot_frame - sensed_force_offset); // flip sign of robot's sensed force + start at zero force
			...
			prev_delayed_force_space_dimension = delayed_force_space_dimension;
		}
