...
static const string world_file = "./resources/world_probe.urdf";
static const string robot_file = "./resources/panda_arm.urdf";
static const string robot_name = "panda";
static const string haptic_proxy_name = "haptic_proxy";
static const string camera_name = "camera";
static const string link_name = "end_effector"; //robot end-effector
...
int main() {
	...
	for (unsigned int i = 0; i < graphics->_world->getNumChildren(); ++i) {
		if (haptic_proxy_name == graphics->_world->getChild(i)->m_name) {
			graphics->_world->getChild(i)->setShowFrame(true);
		}
	}
	...
	static const Vector3d pos_in_link = Vector3d(0.0, 0.0, 0.23);
	Vector3d haptic_proxy = Vector3d::Zero();
	Matrix3d haptic_proxy_rot = Matrix3d::Identity();
	Quaterniond haptic_proxy_rot_quat = Quaterniond::Identity();
	robot->position(haptic_proxy, link_name, pos_in_link);
	robot->rotation(haptic_proxy_rot, link_name);
	haptic_proxy_rot_quat = haptic_proxy_rot;

	// Add force sensor to the end-effector
	Affine3d sensor_transform_in_link = Affine3d::Identity();
    static const Vector3d sensor_pos_in_link = Eigen::Vector3d(0.0,0.0,0.034);
	...
	while (!glfwWindowShouldClose(window) && fSimulationRunning)
	{
		...
		robot->rotation(haptic_proxy_rot, link_name);
		haptic_proxy_rot_quat = haptic_proxy_rot;
		graphics->updateObjectGraphics(haptic_proxy_name, haptic_proxy, haptic_proxy_rot_quat);
